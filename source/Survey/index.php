...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    function error_check($postKey, $postVar, &$columns, &$values) {
        if (empty($postVar)){
    ...
    $postKeys = array_keys($cleanPost);
    for($ctr=0;$ctr<count($cleanPost);$ctr++){
        error_check($postKeys[$ctr],$cleanPost[$postKeys[$ctr]],$dbColumns,$dbValues);
    } 

    $smartquery="INSERT INTO survey_results($dbColumns) VALUES ($dbValues)";
    	// execute query and check if success
    if( $db->exec($smartquery) ){
      $message = "<h1>Thank you for submitting your survey!</h1>";
      $vl= $hl= $aid= $sr= $dso= $inter= $outd= $wp= $ir= $cane= $anim= $help= $attd= $lc= $li= $wa= $woa= $iex= $oex= $whlp= $app ='';
    }
    else {
...
<div><?php echo $message;?></div>
  ...
  <form name="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" onsubmit="validate()" method = "POST">
    <fieldset>
      <legend>
      ...
      <fieldset>
        <div>
          ...
          <input value="<?php echo htmlspecialchars($vl); ?>" type="text" id="vision_level" name="vision_level" />
        </div>
      </fieldset>
      <fieldset>  
        <div>
          <label for="how_long">How long have you had a visual impairment?</label>
          <input value="<?php echo htmlspecialchars($hl);?>" type="text" name="how_long" id="how_long" />
        </div>
      </fieldset>
      <fieldset>
        <div>
          <label for="aid">What do you use to help you navigate the University?</label>
          <input value="<?php echo htmlspecialchars($aid);?>" type="text" id="aid" name="aid"/>
        </div>
      </fieldset>
      <fieldset>
        <div>
          <label for="screen_reader">If you use a screen reader, which do you prefer?</label>
          <input value="<?php echo htmlspecialchars($sr);?>" type="text" name="screen_reader" id="screen_reader"/>
        </div>
      </fieldset>
      <fieldset>
        <div>
            <label for="dso">What elements of the Disability Services Office do you use?</label>
          <input value="<?php echo htmlspecialchars($dso);?>" type="text" name="dso" id="dso" />
        </div>
      </fieldset>
    </fieldset>
    <fieldset>
    ...
    <legend><h3>Open-ended Questions</h3></legend>
      <fieldset>
        ...
        <textarea id="without_aid" name="without_aid" cols="80" rows="10"><?php echo htmlspecialchars($woa);?></textarea>
      </fieldset>
      <fieldset>
        <legend>If you travel with an aid or seeing-eye dog, how do you map the trip in your mind?</legend>
        <label for="with_aid"></label>
        <textarea id="with_aid" name="with_aid" cols="80" rows="10"><?php echo htmlspecialchars($wa);?></textarea>
      </fieldset>
      <fieldset> 
        ...
        <textarea id="inside_experience" name="inside_exp" cols="80" rows="10"><?php echo htmlspecialchars($iex);?></textarea>
      </fieldset>
      <fieldset> 
        ...
        <textarea id="outside_experience" name="outside_exp" cols="80" rows="10"><?php echo htmlspecialchars($oex);?></textarea>
      </fieldset>
      <fieldset>
        <legend>What would you find helpful for navigating the inside of campus buildings?</legend>
        <label for="what_helpful"></label>
        <textarea id="what_helpful" name="what_help" cols="80" rows="10"><?php echo htmlspecialchars($whlp);?></textarea>
      </fieldset>
      <fieldset>
        ...
        <textarea  id="app_desires" name="app_desires" cols="80" rows="10"><?php echo htmlspecialchars($app);?></textarea>
      </fieldset>
    </fieldset>
    <table>
        <tr>
  ...
  <script>
    ...
    function initialize (){
      document.form.onsubmit=validate;
    }
    function validate(){
        alert("Form should be validated");
    }
    function tf(var){
        return ('true' ? var == 1 : 'false');
    }
  </script>
</body>
</html>

...
def run_simple_text(directory):
    all_res = []
    for img in tqdm(os.listdir(directory), desc=f'running the simple mechanism'):
        run_res = TR.get_text_with_image_manipulation(cv2.imread(os.path.join(directory, img)), img)
        all_res.append(run_res)
    # with open('/home/johnny/Documents/text_reading_testing/simple_text_raw_resutls.json', 'w') as f:
    #     json.dump(all_res, f, indent=2)
    df = pandas.DataFrame(all_res)
    df.to_pickle(f"/home/johnny/Documents/text_reading_testing/simple_text_raw_resutls_2_dilations_{datetime.datetime.now().strftime('%Y-%m-%d-%s')}.pkl")
    print('done dude')
...
if __name__ == "__main__":
    ...
    if args.get('images_only'):
        # lazy so just hardcoding it
        # run_text(args['directory'])
        run_simple_text(args['directory'])
